...
if (!FS_Flag) {
	...
	temp = temp & 1;
	/* Concatenation of the CRC bit to Input_Frame : */
	Input_Frame[Fs_N0_Tot + Fs_N1_Tot + Fs_N2_Tot] = temp;
	...
	temp = temp & 1;
	Input_Frame[Fs_N0_Tot + Fs_N1_Tot + Fs_N2_Tot + 1] = temp;
	...
	temp = temp & 1;
	Input_Frame[Fs_N0_Tot + Fs_N1_Tot + Fs_N2_Tot + 2] = temp;
	...
	temp = temp & 1; 
	Input_Frame[Fs_N0_Tot + Fs_N1_Tot + Fs_N2_Tot + 3] = temp;
	return(0);
}
}
/**************************************************************************
...
if (!FS_Flag) {
	...
	for (i = 0; i < N0; i++) 
	   for (j=0; j<SpFrms_per_TDMFrm; j++)
	       Output_Frame[SpFrms_per_TDMFrm*i + j] = 
	                            Input_Frame[j*Length_vocoder_frame + TAB0[i] - 1];

	/* Building and Concatenation of Class 1 : */
	for (i = 0; i < N1; i++)
	   for (j=0; j<SpFrms_per_TDMFrm; j++)
	       Output_Frame[SpFrms_per_TDMFrm*(N0 + i) + j] = 
	                            Input_Frame[j*Length_vocoder_frame + TAB1[i] - 1];

	/* Building and Concatenation of Class 2 : */
	for (i = 0; i < N2; i++)
	   for (j=0; j<SpFrms_per_TDMFrm; j++)
	       Output_Frame[SpFrms_per_TDMFrm*(N0 + N1 + i) + j] = 
	                             Input_Frame[j*Length_vocoder_frame + TAB2[i] - 1];
	...
	for (j=1; j<Fs_SpFrms_per_TDMFrm; j++) 
	    ...
	    for (j=0; j<Fs_SpFrms_per_TDMFrm; j++)
            {
               if (j==0)
		  Output_Frame[k++] = Input_Frame[j*Length_vocoder_frame + Fs_TAB0[j][i] - 1];
               else if ((Fs_TAB0[j][m[j-1]] == Fs_TAB0[0][i]) && (m[j-1] < Fs_N0[j]))
                  Output_Frame[k++] = Input_Frame[j*Length_vocoder_frame + Fs_TAB0[j][m[j-1]++] - 1];
            }
	...
	for (i = 0; i < Fs_N1[0]; i++)
	    for (j=0; j<Fs_SpFrms_per_TDMFrm; j++)
            {
               if (j==0)
		  Output_Frame[k++] = Input_Frame[j*Length_vocoder_frame + Fs_TAB1[j][i] - 1];
               else if ((Fs_TAB1[j][m[j-1]] == Fs_TAB1[0][i]) && (m[j-1] < Fs_N1[j]))
                  Output_Frame[k++] = Input_Frame[j*Length_vocoder_frame + Fs_TAB1[j][m[j-1]++] - 1];
            }
	...
	for (i = 0; i < Fs_N2[0]; i++)
	    for (j=0; j<Fs_SpFrms_per_TDMFrm; j++)
            {
               if (j==0)
		  Output_Frame[k++] = Input_Frame[j*Length_vocoder_frame + Fs_TAB2[j][i] - 1];
               else if ((Fs_TAB2[j][m[j-1]] == Fs_TAB2[0][i]) && (m[j-1] < Fs_N2[j]))
                  Output_Frame[k++] = Input_Frame[j*Length_vocoder_frame + Fs_TAB2[j][m[j-1]++] - 1];
            }
...
if (!FS_Flag) {
	...
	-       Fs_CRC bits
	-       (K - 1) zeros to empty the encoder */
	Nber_Info_Bits = Fs_N1_Tot + Fs_N2_Tot + Fs_SIZE_CRC;
	/* Zero the (K - 1) last bits of the Input_Frame */
	for (i = 0; i < K - 1; i++) {
		Input_Frame[Fs_N0_Tot + Nber_Info_Bits] = 0;
		Nber_Info_Bits++;
		}
	/* Nber_Info_Bits = Fs_N1_Tot + Fs_N2_Tot + Fs_SIZE_CRC + (K - 1) */
