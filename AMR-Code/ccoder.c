...
Word16 FRAME_STEALING=FALSE;    /* TRUE = 10 % of frames */
...

{
	...
	Word16  i;
	Word16  Vocod_array[274+GUARD];       /* Input Buffer : 2 vocoder frames */
	...
	if (( argc < 3 ) || ( argc > 5 ))
	{
		puts( "usage : ccoder infile outfile [CoderType [S]]" );
		...
		else
		{
			if( fread( Vocod_array, sizeof(short), 1, fin ) != 1 ) {
				...
				break;
			     ...
			     /* 1st speech frame */
			if( fread( Vocod_array, sizeof(short), 249, fin ) != 249 ) {
				puts( "chanlcod: reached end of file" );
				break;
			}
			      /* Skip over bfi bit */
			if( fread( Vocod_array+Length_vocoder_frame, sizeof(short), 1, fin ) != 1 ) {
				puts( "chanlcod: reached end of file" );
				break;
			}
			     /* 2nd speech frame */
			if( fread( Vocod_array+Length_vocoder_frame, sizeof(short), 249, fin ) != 249 ) {
				puts( "chanlcod: reached end of file" );
				break;
			}
			      /* Skip over bfi bit */
			if( fread( Vocod_array+2*Length_vocoder_frame, sizeof(short), 1, fin ) != 1 ) {
				puts( "chanlcod: reached end of file" );
				break;
			}
			     /* 3rd speech frame */
			if( fread( Vocod_array+2*Length_vocoder_frame, sizeof(short), 249, fin ) != 249 ) {
				puts( "chanlcod: reached end of file" );
				break;
			}
		}
